...
const jewelryId = async (id) =>
  await genericSqlQuery("SELECT * FROM inventario WHERE id = $1;", [id]);
...
const HATEOAS = (joyas, limit, order, page) => {
  ...
  return {
    ...
    next:
      page < 2
        ? `/joyas?limit=${limit}&order=${order}&page=${+page + 1}`
        : null,
    previous:
      page > 1 ? `/joyas?limit=${limit}&order=${order}&page=${page - 1}` : null,
    results,
  };
};

const allJewels = async ({ limits = 6, page = 1, order_by = "nombre_asc" }) => {
